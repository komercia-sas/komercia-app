  ...
  return (
    <Card className="group overflow-hidden card-shadow hover:shadow-lg transition-all duration-300">
      <div className="relative overflow-hidden">
        ...
        />
        {!product.inStock && (
          <Badge className="absolute top-3 left-3 bg-destructive text-destructive-foreground">Agotado</Badge>
        )}
      ...
      <CardContent className="p-6">
        <div className="space-y-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground text-pretty line-clamp-2">{product.shortDescription}</p>
          </div>

          <div className="flex items-center justify-between">
            <div className="text-2xl font-bold text-primary">{formatPrice(product.price)}</div>
            <Button onClick={handleAddToCart} disabled={!product.inStock} className="btn-primary" size="sm">
